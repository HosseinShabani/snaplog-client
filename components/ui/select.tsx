...
>(({ className, children, ...props }, ref) => (
  <SelectPrimitive.Trigger
    ref={ref}
    className={cn(
      'flex flex-row h-12 native:h-12 items-center text-sm justify-between rounded-lg border border-gray-20 bg-background px-3 py-2 web:ring-offset-background text-muted-foreground web:focus:border-gray-60 web:focus:ring-ring web:focus:ring-offset-0 [&>span]:line-clamp-1',
  ...
  >
    <>{children}</>
    <ChevronDown size={20} aria-hidden={true} className="typo-[20] text-black" />
  </SelectPrimitive.Trigger>
));
      ...
      <SelectPrimitive.Overlay style={Platform.OS !== 'web' ? StyleSheet.absoluteFill : undefined}>
        <Animated.View entering={FadeIn} exiting={FadeOut}>
          <SelectPrimitive.Content
            ref={ref}
            className={cn(
              'relative z-50 max-h-96 mt-2 min-w-[8rem] rounded-md border border-border bg-popover shadow-md shadow-foreground/10 py-2 px-1 data-[side=bottom]:slide-in-from-top-2 data-[side=left]:slide-in-from-right-2 data-[side=right]:slide-in-from-left-2 data-[side=top]:slide-in-from-bottom-2',
              position === 'popper' &&
